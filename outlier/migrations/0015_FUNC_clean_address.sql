...
-- replace punctuation
  input := upper(input);
	...
	input := regexp_replace(input, '\mWEST\M', 'W', 'g');
	input := regexp_replace(input, '\mEAST\M', 'E', 'g');
	input := regexp_replace(input, '\mNORTH\M', 'N', 'g');
	input := regexp_replace(input, '\mSOUTH\M', 'S', 'g');
